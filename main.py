...
def get_html_layout(content: str, active_tab: str):
    ...
    <html>
        ...
        <body>
            <div class="nav"><a href="/" style="{home_style}">MY LIST</a><a href="/archived" style="{archive_style}">ARCHIVE</a></div>
...
async def index():
    ...
    content = f"""
    <div class="card">
        <h3 style="margin:0 0 10px; color:#ef4056;">🤖 AI要約</h3>
        ...
        <form action="/ai-collect" method="post" style="border-top:1px solid #eee; padding-top:15px;">
            <textarea name="urls" rows="2" style="width:100%; padding:10px; margin-bottom:10px;" placeholder="URL（任意）"></textarea>
    ...
    for a in articles:
        ...
        <div class="card">
            ...
            <div style="display:flex; gap:10px; margin-top:10px;">
                <form action="/archive/{a['id']}" method="post"><button style="color:#ef4056; background:none; border:none; font-weight:bold;">Done</button></form>
                <form action="/delete/{a['id']}" method="post"><button style="color:#ccc; background:none; border:none;">×</button></form>
            </div>
        </div>
        """
...
async def ai_collect(urls: str = Form(""), count: int = Form(5)):
    try:
        ...
        for item in search_results['results']:
            supabase.table("articles").insert({"title": item['title'], "url": item['url'], "is_archived": False, "ai_reason": "AIサーチ"}).execute()
        return RedirectResponse(url="/", status_code=303)
    except:
        return RedirectResponse(url="/", status_code=303)
