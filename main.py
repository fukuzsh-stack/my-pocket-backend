...
async def index():
    ...
    try:
        ...
        for a in articles:
            ...
            img = f'<img src="{{a["image_url"]}}" class="thumb">' if a.get('image_url') else '<div style="width:55px;"></div>'
            rows += f"""
            <div class="list-item" data-title="{{a['title']}}" data-domain="{{domain}}">
                {{img}}
                <div class="item-content">
                    <div class="item-meta"><img src="{{favicon}}" class="favicon"> {{domain}} • {{time_ago(a.get('created_at',''))}}</div>
            ...
            """
        return get_layout(rows if rows else '<div style="padding:40px; text-align:center; color:#8e8e93;">記事はありません</div>', "unread")
    except Exception as e:
